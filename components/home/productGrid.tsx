...
export function ProductGrid() {
  ...
  const { products, loading: productsLoading } = useProducts({ categoryId: activeCategoryId })
...
//clicks fuera del panel 
  ...
  return (
    <section className="px-6 py-8 lg:px-50">
      <div className="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {categories.slice(0, categoryVisibles).map((category) => (
           ...
           {activeCategoryId && !categories.slice(0, categoryVisibles).find(c => c.id === activeCategoryId) && (
            <Button
              type="button"
              onClick={() => setActiveCategoryId(undefined)}
          ...
          <div className="fixed  right-4 top-50 z-50 md:absolute md:left-auto md:right-0 md:top-12 md:w-64 md:fixed-none">
            <FiltroPanel
              ...
              onFilterChange={({ categories: cats }) => {
                ...
                else if (cats.length === 0) setActiveCategoryId(undefined)
              }}
            />
          </div>
        )}
